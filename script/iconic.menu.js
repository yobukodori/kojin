...
(function(){
	...
	iconicMenu.items.forEach(item=>{
			...
			e.innerHTML = '<a href="' + item.url + '" '+(targetBlank ? 'target="_blank"':'')+'><div><img class="item-icon" src="' + (item.icon ? item.icon : 'icon/na.png')+ '"><div class="item-name">' + item.name + '</div></div></a>';
			e.className = "item";
